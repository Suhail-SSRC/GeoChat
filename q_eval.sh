...
#Scene Calssification 4 bits UCMerced
#python geochat/eval/batch_geochatq_scene.py --model-path MBZUAI/geochat-7B --question-file datasets/GeoChat-Bench/UCmerced.jsonl --answers-file datasets/UCMerced/ansQVLM_4bit.json --image-folder-calibrate datasets/UCMerced/Images/train --question-file-calibrate datasets/UCmerced/UCmerced_train.json  --load-4bit | tee UC4bits.txt


#Scene Calssification 4 bits AID
#python geochat/eval/batch_geochatq_scene.py --model-path MBZUAI/geochat-7B --question-file datasets/GeoChat-Bench/aid.jsonl --answers-file datasets/AID/ansQVLM_4bit.json --image-folder-calibrate datasets/AID/train/ --question-file-calibrate datasets/LBERN/lbern_train.json --load-4bit | tee AID4bits.txt


#Region Caption 4 bit
#python geochat/eval/batch_geochatq_grounding.py --model-path MBZUAI/geochat-7B --question-file datasets/GeoChat-Bench/region_captioning.jsonl --answers-file datasets/rc4bits.json --image-folder datasets/GeoChat_Instruct/ --load-4bit | tee rc4bits.txt

# Refering (Visual Grounding)

# 4bits
#python geochat/eval/batch_geochatq_referring.py --model-path MBZUAI/geochat-7B --question-file datasets/GeoChat-Bench/referring.jsonl --answers-file datasets/vg4bits.json --image-folder datasets/GeoChat_Instruct/ --load-4bit | tee vg4bits.txt
